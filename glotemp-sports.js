...
// deploys:
//   - TheSportsDB (football/rugby/cricket clubs, resolved by name search --
//     no numeric team IDs are hardcoded anywhere in this codebase)
//   - Jolpica-F1 (Ergast-compatible F1 calendar, ergast.com's maintained
//     successor)
// Every request is wrapped so a network failure, CORS block, or empty
// result just leaves the section blank -- never a broken layout, never
// fabricated scores.
(function () {
  'use strict';
  ...
  async function resolveTeamId(teamName) {
    var cacheKey = 'gt-sports-id:' + teamName;
  ...
  async function loadTeamFixtures(sportKey, teamName) {
    var resolved = await resolveTeamId(teamName);
  ...
  function fixtureBlockHTML(sportKey, fixture) {
    var meta = SPORT_META[sportKey] || { icon: '🏆', label: sportKey };
    var parts = [];
    parts.push('<div class="sport-live-item">');
    parts.push('<p class="sport-live-heading">' + meta.icon + ' <span>' + esc(meta.label) + ' &middot; ' + esc(fixture.teamName) + '</span></p>');
  ...
  async function mount(container) {
    ...
    Object.keys(config).forEach(function (sportKey) {
      (config[sportKey] || []).forEach(function (teamName) {
        teamJobs.push(
          loadTeamFixtures(sportKey, teamName).then(function (fixture) {
            if (fixture && (fixture.next || fixture.last)) blocks.push({ order: 1, html: fixtureBlockHTML(sportKey, fixture) });
          }).catch(function () {})
        );
      });
    });
    ...
    var f1Job = loadF1().then(function (f1) {
      ...
      blocks.push({ order: hosted ? 0 : 2, html: f1BlockHTML(f1, hosted) });
    }).catch(function () {});
