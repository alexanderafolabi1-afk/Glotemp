#!/usr/bin/env node
// Generate /rankings/<vertical>/index.html for all 12 verticals: a live
// league table, ranked by that vertical's primary metric, gated by a
// stated minimum-signal threshold so thin data never masquerades as a
// ranking.
const fs = require('fs');
const path = require('path');

const SIGNAL_THRESHOLD = 2;

const VERTICALS = [
  { slug: 'pulse', label: '💓 Pulse', metric: 'sentiment_score', unit: '/ 10', desc: 'How every city feels right now, ranked.' },
  { slug: 'tech', label: '💻 Tech', metric: 'developer_activity', unit: 'activity index', desc: 'Where the developer and startup activity is highest.' },
  { slug: 'finance', label: '💰 Finance', metric: 'currency_strength', unit: '/ 10', desc: 'Currency stability and cost dynamics, ranked.' },
  { slug: 'work', label: '💼 Work', metric: 'salary_competitiveness', unit: '/ 100', desc: 'Where salaries and work culture are most competitive.' },
  { slug: 'property', label: '🏠 Property', metric: 'property_appreciation', unit: '% annual', desc: 'Where real estate is appreciating fastest.' },
  { slug: 'education', label: '🎓 Education', metric: 'education_quality_score', unit: '/ 10', desc: 'University density and education quality, ranked.' },
  { slug: 'sport', label: '⚽ Sport', metric: 'active_participation', unit: '% participation', desc: 'Where sports participation and venues are strongest.' },
  { slug: 'entertainment', label: '🎭 Entertainment', metric: 'nightlife_score', unit: '/ 10', desc: 'Nightlife energy and cultural events, ranked.' },
  { slug: 'fashion', label: '👗 Fashion', metric: 'style_influence', unit: '/ 10', desc: 'Global style influence and fashion presence, ranked.' },
  { slug: 'food', label: '🍽️ Food', metric: 'culinary_diversity', unit: '/ 10', desc: 'Culinary diversity and dining scenes, ranked.' },
  { slug: 'health', label: '🩺 Health', metric: 'wellness_index', unit: '/ 10', desc: 'Wellness, air quality and healthcare, ranked.' },
  { slug: 'transport', label: '🚇 Transport', metric: 'transit_quality', unit: '/ 10', desc: 'Public transit and mobility quality, ranked.' },
];

function pageHTML(v) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover" />
  <meta name="theme-color" content="#06060A" />
  <meta name="description" content="${v.desc} Live league table across every city Glotemp tracks, ranked by ${v.metric.replace(/_/g, ' ')}.">
  <meta property="og:title" content="${v.label} Rankings - Glotemp" />
  <meta property="og:description" content="${v.desc}" />
  <meta property="og:type" content="website" />
  <meta property="og:url" content="https://glo-temp.com/rankings/${v.slug}/" />
  <meta property="og:image" content="https://glo-temp.com/assets/logo.png" />
  <meta name="twitter:card" content="summary_large_image" />
  <link rel="canonical" href="https://glo-temp.com/rankings/${v.slug}/" />
  <link rel="alternate" hreflang="en" href="https://glo-temp.com/rankings/${v.slug}" />
  <link rel="alternate" hreflang="es" href="https://glo-temp.com/rankings/${v.slug}" />
  <link rel="alternate" hreflang="fr" href="https://glo-temp.com/rankings/${v.slug}" />
  <link rel="alternate" hreflang="de" href="https://glo-temp.com/rankings/${v.slug}" />
  <link rel="alternate" hreflang="pt" href="https://glo-temp.com/rankings/${v.slug}" />
  <link rel="alternate" hreflang="ja" href="https://glo-temp.com/rankings/${v.slug}" />
  <link rel="alternate" hreflang="x-default" href="https://glo-temp.com/rankings/${v.slug}" />
  <link rel="manifest" href="/manifest.json" />
  <link rel="icon" type="image/png" sizes="192x192" href="/assets/icon-192.png" />
  <link rel="icon" type="image/png" sizes="32x32" href="/assets/favicon.png" />
  <link rel="apple-touch-icon" sizes="180x180" href="/assets/apple-touch-icon.png" />
  <title>${v.label} Rankings - Glotemp</title>

  <script type="application/ld+json">
  {
    "@context": "https://schema.org",
    "@type": "ItemList",
    "name": "${v.label} Rankings",
    "description": "${v.desc}",
    "url": "https://glo-temp.com/rankings/${v.slug}"
  }
  </script>

  <link rel="preconnect" href="https://fonts.googleapis.com" />
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
  <link rel="preconnect" href="https://hnysztednzqfzbmiqqgl.supabase.co" />
  <link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght,SOFT,WONK@0,9..144,100..900,0..100,0..1&family=Manrope:wght@400..700&family=IBM+Plex+Mono:wght@400;500&display=swap" rel="stylesheet" />
  <link rel="stylesheet" href="/styles.css?v=58" />
  <style>
    .rank-row {
      position: relative;
      display: flex;
      align-items: center;
      gap: 1.2rem;
      padding: 1rem 1.3rem;
      border-bottom: 1px solid var(--hairline);
      text-decoration: none;
      overflow: hidden;
    }
    .rank-row:last-child { border-bottom: none; }
    /* Translucent light-band, tapering left to right behind the row --
       see mood-resonance-field.png. A pseudo-element so it never
       interferes with the row's own real background. */
    .rank-row::before {
      content: '';
      position: absolute;
      inset: -8px 0;
      background:
        radial-gradient(70% 170% at 6% 50%, rgba(154, 115, 255, 0.6) 0%, transparent 68%),
        radial-gradient(45% 130% at 34% 40%, rgba(70, 226, 214, 0.5) 0%, transparent 62%);
      filter: blur(12px);
      opacity: 0.5;
      transition: opacity 300ms ease;
      pointer-events: none;
    }
    .rank-row:hover::before { opacity: 1; }
    .rank-pos {
      position: relative;
      font-family: var(--font-mono);
      font-size: 1.1rem;
      color: var(--sand);
      width: 2.5rem;
      flex-shrink: 0;
    }
    .rank-city { position: relative; flex: 1; min-width: 0; }
    .rank-city-name { color: var(--ivory); font-family: var(--font-display); font-size: 1.1rem; display: inline-flex; align-items: center; gap: 0.35rem; }
    .rank-city-country { color: var(--sand); font-size: 0.8rem; }
    .rank-value { position: relative; font-family: var(--font-mono); font-size: 1.2rem; text-align: right; flex-shrink: 0; }
    /* Small gold pin mark instead of a plain arrow glyph, same
       arrow-pin language as the reference light-band diagram. */
    .rank-movement { position: relative; font-family: var(--font-mono); font-size: 0.78rem; color: var(--sand); width: 4.5rem; text-align: right; flex-shrink: 0; }
    .threshold-note { padding: 0 1.5rem 1.5rem; color: var(--sand); font-size: 0.85rem; font-family: var(--font-mono); }
  </style>
  <script src="/glotemp-analytics.js" defer></script>
</head>
<body>
  <nav id="site-nav"></nav>
  <script src="/nav-component.js"></script>
  <script src="/footer-social.js" defer></script>

  <main class="page-main">
    <section class="glass-card page-header">
      <p class="eyebrow">Live league table</p>
      <h1>${v.label} Rankings</h1>
      <p class="section-copy" style="color:var(--sand); max-width:64ch;">${v.desc}</p>
    </section>

    <section class="glass-card" style="padding:0; overflow:hidden;">
      <div id="rankings-list"></div>
      <div id="rankings-empty" class="empty-state" style="display:none; margin:1.5rem;"></div>
      <p class="threshold-note">Rankings publish only for cities with at least ${SIGNAL_THRESHOLD} recorded signals for this vertical. Movement compares the trailing 7 days against the 7 days before that.</p>
    </section>

    <section class="glass-card" style="text-align:center; padding:2rem;">
      <p style="color:var(--sand); margin-bottom:1rem;">See every vertical for one city instead.</p>
      <a href="/verticals/${v.slug}.html" class="secondary-link">Explore ${v.label} in detail</a>
    </section>
  </main>

  <footer class="footer glass">
    <p>© 2026 RenviaIT Ltd. All rights reserved.</p>
    <p class="small-print">
      <a href="/privacy">Privacy Policy</a> | <a href="/terms">Terms of Service</a> | <a href="#" id="cookie-settings">Cookie Settings</a>
    </p>
    <p class="small-print"><a href="mailto:info@glo-temp.com" class="contact-link">info@glo-temp.com</a></p>
    <p class="small-print footer-lang-row">
      <button type="button" id="lang-switch" class="footer-lang">Language</button>
    </p>
  </footer>

  <script src="/cities-data.js"></script>
  <script src="/glotemp-core.js"></script>
  <script src="/city-landmarks.js"></script>
  <script src="/city-landmark-photos.js"></script>
  <script>
    // var, not const: tempo-economy.js (loaded below) also declares
    // these; var redeclaration across scripts is a safe no-op.
    var SUPABASE_URL = SUPABASE_URL || 'https://hnysztednzqfzbmiqqgl.supabase.co';
    var SUPABASE_ANON_KEY = SUPABASE_ANON_KEY || 'sb_publishable_AV3IDw0gfEnwf4ZSTYQPRQ_tzDogHi_';
    const VERTICAL = '${v.slug}';
    const METRIC = '${v.metric}';
    const THRESHOLD = ${SIGNAL_THRESHOLD};

    async function loadRankings() {
      const listEl = document.getElementById('rankings-list');
      const emptyEl = document.getElementById('rankings-empty');
      try {
        const resp = await fetch(
          \`\${SUPABASE_URL}/rest/v1/readings?vertical=eq.\${VERTICAL}&select=city_slug,metric,value,fetched_at&order=fetched_at.desc&limit=3000\`,
          { headers: { apikey: SUPABASE_ANON_KEY, Authorization: \`Bearer \${SUPABASE_ANON_KEY}\`, Accept: 'application/json' } }
        );
        if (!resp.ok) throw new Error('fetch failed');
        const rows = await resp.json();
        const relevant = rows.filter(r => r.metric === METRIC && typeof r.value === 'number');

        const byCity = new Map();
        relevant.forEach(r => {
          if (!byCity.has(r.city_slug)) byCity.set(r.city_slug, []);
          byCity.get(r.city_slug).push(r);
        });

        const dayMs = 86400000;
        const now = Date.now();
        const ranked = [];
        byCity.forEach((readings, slug) => {
          if (readings.length < THRESHOLD) return;
          const avg = readings.reduce((s, r) => s + r.value, 0) / readings.length;
          const recent = readings.filter(r => now - new Date(r.fetched_at).getTime() < 7 * dayMs);
          const prior = readings.filter(r => {
            const age = now - new Date(r.fetched_at).getTime();
            return age >= 7 * dayMs && age < 14 * dayMs;
          });
          let movement = null;
          if (recent.length && prior.length) {
            const a = arr => arr.reduce((s, r) => s + r.value, 0) / arr.length;
            movement = a(recent) - a(prior);
          }
          const city = window.CITIES_DATA.find(c => c.slug === slug);
          ranked.push({ slug, city, value: avg, movement, signals: readings.length });
        });

        ranked.sort((a, b) => b.value - a.value);

        if (!ranked.length) {
          listEl.innerHTML = '';
          emptyEl.style.display = 'block';
          emptyEl.textContent = 'No city has reached the ' + THRESHOLD + '-signal threshold for ${v.label} yet.';
          return;
        }

        listEl.innerHTML = ranked.map((r, i) => {
          const name = r.city ? r.city.name : r.slug;
          const country = r.city ? r.city.country : '';
          const moveLabel = r.movement === null ? '-' :
            (r.movement > 0.05 ? '▲ ' + r.movement.toFixed(1) : r.movement < -0.05 ? '▼ ' + Math.abs(r.movement).toFixed(1) : '- steady');
          const color = r.city ? GlotempCore.moodToBand(r.city.mood).color : 'var(--band-equilibrium)';
          return \`<a class="rank-row hover-lift" href="/cities/\${r.slug}/${v.slug}/" data-city-link="\${r.slug}" data-city-nav="false" style="--hover-band:\${color};">
            <span class="rank-pos">#\${i + 1}</span>
            <span class="rank-city">
              <span class="rank-city-name" style="color:\${color};">\${typeof GlotempLandmarks !== 'undefined' ? GlotempLandmarks.cityIconHTML(r.slug, { size: 15, className: 'city-landmark-icon' }) : ''}\${name}</span><br>
              <span class="rank-city-country">\${country} · \${r.signals} signal\${r.signals === 1 ? '' : 's'}</span>
            </span>
            <span class="rank-value" style="color:\${color};">\${r.value.toFixed(1)} ${v.unit}</span>
            <span class="rank-movement">\${moveLabel}</span>
          </a>\`;
        }).join('');

        listEl.querySelectorAll('.rank-row').forEach(row => {
          row.addEventListener('click', (e) => {
            const slug = row.getAttribute('data-city-link');
            GlotempCore.setPinnedCity(slug);
            window.location.href = row.getAttribute('href');
          });
          if (typeof GlotempLandmarkPhotos !== 'undefined') {
            const slug = row.getAttribute('data-city-link');
            const nameEl = row.querySelector('.rank-city-name');
            if (slug && nameEl) GlotempLandmarkPhotos.upgrade(nameEl, slug, 15);
          }
        });
      } catch (e) {
        console.warn('Rankings unavailable:', e.message);
        listEl.innerHTML = '';
        emptyEl.style.display = 'block';
        emptyEl.textContent = 'Rankings are temporarily unavailable. Try again shortly.';
      }
    }

    loadRankings();
    const lang = localStorage.getItem('glotemp-lang') || 'en';
    document.documentElement.lang = lang;
  </script>
  <script src="/cookie-consent.js" defer></script>
  <script src="/tempo-economy.js" defer></script>
</body>
</html>
`;
}

const outDir = path.join(__dirname, 'rankings');
let count = 0;
VERTICALS.forEach(v => {
  const dir = path.join(outDir, v.slug);
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, 'index.html'), pageHTML(v));
  count++;
});
console.log(`✅ Generated ${count} ranking pages in /rankings/<vertical>/`);
